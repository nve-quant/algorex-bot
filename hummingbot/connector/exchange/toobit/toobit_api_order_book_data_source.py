...
class ToobitAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot_timestamp: float = int(snapshot_response["t"]) * 1e-3
        update_id: int = int(snapshot_timestamp)

        order_book_message_content = {
            "trading_pair": trading_pair,
            "update_id": update_id,
            "bids": [(bid[0], bid[1]) for bid in snapshot_response["b"]],
            "asks": [(ask[0], ask[1]) for ask in snapshot_response["a"]],
        }
    ...
    async def _request_order_book_snapshot(self, trading_pair: str) -> Dict[str, Any]:
        ...
        data = await rest_assistant.execute_request(
            url=web_utils.public_rest_url(path_url=CONSTANTS.TOOBIT_ORDER_BOOK_PATH),
            params=params,
            method=RESTMethod.GET,
            throttler_limit_id=CONSTANTS.TOOBIT_ORDER_BOOK_PATH,
        )
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        async with self._api_factory.throttler.execute_task(limit_id=CONSTANTS.WS_CONNECTION_LIMIT_ID):
            await ws.connect(
                ws_url=CONSTANTS.TOOBIT_WS_URI_PUBLIC,
