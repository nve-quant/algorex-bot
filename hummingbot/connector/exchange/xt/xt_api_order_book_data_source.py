...
from hummingbot.connector.exchange.xtcom import xtcom_constants as CONSTANTS, xtcom_web_utils as web_utils
from hummingbot.connector.exchange.xtcom.xtcom_order_book import XtcomOrderBook
...
if TYPE_CHECKING:
    from hummingbot.connector.exchange.xtcom.xtcom_exchange import XtcomExchange


class XtcomAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot_msg: OrderBookMessage = XtcomOrderBook.snapshot_message_from_exchange(
            snapshot,
            snapshot_timestamp,
    ...
    async def _parse_trade_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        if "result" not in raw_message:
            ...
            trade_message = XtcomOrderBook.trade_message_from_exchange(
    ...
    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        if "result" not in raw_message:
            trading_pair = await self._connector.trading_pair_associated_to_exchange_symbol(symbol=raw_message.get("params", {}).get("symbol"))
            order_book_message: OrderBookMessage = XtcomOrderBook.diff_message_from_exchange(
